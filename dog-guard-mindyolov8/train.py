...
# --- 修改点 1: 导入 VOCDataset ---
# 假设 VOCDataset 在 mindyolo.data.voc_dataset 模块中
# 请根据实际的模块结构调整导入路径
try:
    from mindyolo.data.voc_dataset import VOCDataset
    HAS_VOCDATASET = True
except ImportError:
    HAS_VOCDATASET = False
    print("[WARNING] VOCDataset could not be imported. VOC dataset support might be unavailable.")

from mindyolo.data import COCODataset, create_loader # COCODataset 已在此处导入
...
def train(args):
    ...
    # --- 修改点 2: 动态选择 Dataset 类 ---
    # 获取数据集类型名称，默认为 'coco'
    dataset_type = getattr(args.data, 'dataset_name', 'coco').lower()

    # 定义一个函数来根据类型获取 Dataset 类
    def get_dataset_class(name):
        if name == 'coco':
            ...
            if HAS_VOCDATASET:
                ...
                raise ValueError(f"Dataset type '{name}' is specified, but 'VOCDataset' could not be imported. Please check your installation or implementation.")
        else:
            raise ValueError(f"Unsupported dataset type: {name}. Supported types: ['coco', 'voc']")

    # 获取对应的 Dataset 类
    try:
        DatasetClass = get_dataset_class(dataset_type)
        logger.info(f"Selected dataset class: {DatasetClass.__name__} for type '{dataset_type}'")
    except ValueError as e:
        logger.error(f"Error selecting dataset class: {e}")
        raise
    ...
    #     args.network.recompute = args.recompute
    # else:
    #     print("[WARNING] args.network is None. Skipping recompute setting.")
    ...
    # --- 修改点 3: 使用动态选择的 DatasetClass ---
    for stage in range(len(stage_epochs)):
        _dataset = DatasetClass( # <-- 使用动态选择的类
    ...
    # --- 修改点 4: 验证集也使用动态选择的 DatasetClass ---
    if args.run_eval:
        from test import test
        eval_dataset = DatasetClass( # <-- 使用动态选择的类
            ...
            batch_size=args.per_batch_size,
            stride=max(args.network.stride),
            # 注意：测试集通常不需要 return_segments，除非验证过程也需要分割 mask
            # return_segments=(args.task == "segment") 
        )
    ...
    if args.run_eval:
        ...
        # 注意：这里判断 is_coco_dataset 的逻辑可能需要根据新数据集调整
        is_coco_dataset = (
        ...
        test_fn = partial(
            ...
            ), # <-- 这个 JSON 路径仅适用于 COCO，对于 VOC 需要不同处理或禁用基于 JSON 的评估
